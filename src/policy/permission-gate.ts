...
export type PermissionGateResult =
  | {
      ...
      /** Fork: set when the human approved forever, carrying what to record. */
      foreverApproval?: { grants: readonly { surface: string; pattern: string }[] };
    }
...
export async function applyPermissionGate(
  ...
  if (state === "ask") {
    ...
    if (decision.state === "approved_forever" && params.foreverApproval) {
      return { action: "allow", decidedBy, foreverApproval: params.foreverApproval };
    }
